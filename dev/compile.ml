...
let rec compile_expr (e : tag eexpr) (env : reg_env) (fenv : funenv) : instruction list =
  ...
  | EPrim2 (op, e1, e2, tag) -> 
    ...
    let lazy_eval (inst : instruction list) (test) (value : int64) : instruction list =
      (compile_expr e2 env fenv) @ (test RAX 2 tag) @ (* evalua un operando *)
      ...
      (compile_expr e1 env' fenv) @ (test RAX 1 tag) @ inst @ [ILabel jump_label] in
  ...
  | EApp (f, p, _) -> 
    ...
    let compile_elist (exprs : tag eexpr list) : instruction list list =
      ...
      begin match arg_n with
        | Some n -> if (n == List.length p) then (caller_instrs f (compile_elist p))
          else failwith(sprintf "Arity mismatch: %s expected %d arguments but got %d
          " f n (List.length p))
...
let compile_function (func : tag efundef) (env : funenv) : instruction list * funenv =
  match func with
  | EDefFun (fun_name, arg_list, e, _) -> 
    callee_instrs fun_name (compile_expr (e) (env_from_args arg_list) env) 100, ((fun_name, List.length arg_list)::env)
  | _ -> failwith("TODO")
...
let compile_prog (p : prog) : string =
  let tag_flist, tagged = tag_eprogram p in
